...
export const AdminRequirementsPage = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('تم إضافة المتطلب')
      setForm(EMPTY_FORM)
      invalidate()
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('تم تحديث المتطلب')
      setForm(EMPTY_FORM)
      setEditingId(null)
      invalidate()
    },
  ...
  const handleDelete = (item) => {
    if (window.confirm(`حذف المتطلب «${item.title}»؟`)) {
  ...
  return (
    <div dir="rtl">
      <PageHeader
        ...
        description="إدارة المستندات والشروط — تظهر في صفحة /requirements للزوار"
      />

      <div className="grid gap-loose xl:grid-cols-[1fr_400px]">
        <Card title="قائمة المتطلبات" padding="none">
          {requirementsQuery.isLoading ? (
            ...
            <>
              <DataTable columns={columns} rows={paginated} emptyLabel="لا متطلبات بعد" />
        ...
        <Card
          title={editingId ? 'تعديل متطلب' : 'إضافة متطلب'}
          className="xl:sticky xl:top-24 xl:self-start"
        >
          <form onSubmit={handleSubmit}>
            <FormSection description="المتطلبات النشطة فقط تظهر للزوار مرتبة حسب رقم الترتيب">
              <Input
                label="العنوان"
                value={form.title}
              ...
              <Input
                ...
                hint="مثال: badge، health_and_safety"
              />
              <Input
              ...
              <Input
                ...
                hint="مثال: /images/driving/license.jpg"
              />
              <Input
                label="ترتيب العرض"
                type="number"
